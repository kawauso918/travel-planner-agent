...
- **PDF/ICS出力**: 生成された旅程をPDFやカレンダー形式（ICS）でダウンロード可能
- **地図リンク**: 各スポットのGoogle Mapsリンクを自動生成・表示
- **Judge評価**: LLM as a Judgeによる旅程の実用性評価（移動の無理がないか、時間配分の妥当性、重複や矛盾がないか）
...
5. **エクスポート機能**
   - PDF出力: 日本語フォント対応、地図リンク付き
   - ICS出力: カレンダーアプリにインポート可能、地図リンク付き
   - 地図リンク: 各スポットのGoogle Mapsリンクを自動生成

6. **Judge評価機能**
   - 旅程の実用性を6つの観点から評価（適切性、実現可能性、情報の正確性、構造の完全性、予算の妥当性、注意点の適切性）
   - 改善点の提案とフィードバック
...
1. **旅行条件の入力**
   - サイドバーから目的地、滞在日数、予算、興味テーマなどを入力
   - 「🚀 旅程を生成」ボタンをクリック

2. **旅程の確認**
   - 生成された旅程を確認
   - Judge評価結果で実用性を確認（自動実行）
   - 改善点があれば「🔄 別案を生成（再生成）」で再生成

3. **エクスポート**
   - PDF: 「📄 PDF出力」ボタンでPDFをダウンロード（地図リンク付き）
   - ICS: 「📅 ICS出力」ボタンでカレンダーファイルをダウンロード（地図リンク付き）
   - 地図リンク: 「🗺️ 地図リンク」ボタンで各スポットのGoogle Mapsリンクを表示

4. **旅程の編集**
   - 「✏️ この旅程を修正」ボタンで修正指示を入力
   - 修正後の旅程が自動的に更新されます
...
**エクスポート機能の活用：**
- PDF出力: 旅程をPDF形式で保存（地図リンク付き、日本語フォント対応）
- ICS出力: カレンダーアプリ（Google Calendar、Apple Calendar等）にインポート可能
- 地図リンク: 各スポットのGoogle Mapsリンクをクリックして地図を確認

**Judge評価機能の活用：**
- 旅程生成後に自動的に評価が実行されます
- 6つの観点（適切性、実現可能性、情報の正確性、構造の完全性、予算の妥当性、注意点の適切性）から評価
- 改善点が表示されるので、それを参考に再生成できます
...
ICSファイルには以下の情報が含まれます：
- 各日の終日イベント（Day1, Day2...）
- イベント説明にスポット一覧と地図リンクを含む
- カレンダーアプリにインポート可能
...
評価結果には以下の情報が含まれます：
- 総合スコア（0-100点）
- 各観点のスコア（適切性、実現可能性、情報の正確性、構造の完全性、予算の妥当性、注意点の適切性）
- フィードバック
- 強みリスト
- 改善点リスト

**実現可能性（Feasibility）の評価内容**：
...
- 日本語フォントが必要です。`fonts/NotoSansJP-Regular.ttf`を配置するか、システムフォントを使用してください
- 地図リンクはPDF内にURLとして表示されます（クリック可能）

### ICS出力について

- 開始日を指定すると、その日から旅程がカレンダーに追加されます
- 各日は終日イベントとして追加されます
- イベント説明に地図リンクが含まれます
...
- 評価は旅程生成後に自動的に実行されます
- 評価にはOpenAI APIを使用するため、追加のAPIコストが発生します
- 評価結果は参考情報として活用してください
- **実用性を重視**: 実際に旅行を実行できるかどうかを厳密に評価します
  - 移動の無理がないか（距離/回数/順序の自然さ）
  - 時間配分の妥当性（詰め込みすぎ/スカスカ）
  - 重複や矛盾がないか（同じ場所連発、営業時間無視など）
- 改善点があれば「🔄 別案を生成（再生成）」ボタンで再生成できます

### 引用の扱いについて

- **検索→引用→日程表→出力の一貫性**: 検索結果に基づく情報には、可能な限り引用元（URL）が明記されます
- **引用元の明記**: 各スポットの説明に「（[出典URL]より）」や「📚 参照: [URL]」の形式で明記されます
- **参照リンクセクション**: 最終出力の「📚 参照リンク」セクションに、使用したすべてのURLがリストアップされます
- **要確認の明記**: 検索していない情報は「要確認」と明記されます
- **根拠の可視化**: 引用がない情報は「要確認」と明記され、引用がある情報は出典URLが明記されます
